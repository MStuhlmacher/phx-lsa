...
#open pdf file to save plot to
#pdf("FIG7.pdf",width=11,height=7)

figure=ggarrange(ndviA,ndviB,lstA,lstB,ndviC,ndviD,lstC,lstD,ncol=4,nrow=2,common.legend = TRUE, legend="bottom")+
    theme(
...
annotate_figure(figure,
    ...
    left = text_grob("", rot = 90, size = 14),
    right = text_grob("", rot = 90, size = 14))+
    theme(panel.background = element_rect(fill = "grey90",colour = "grey90"),
          panel.border = element_rect(colour = "white", fill=NA, size=1),
    plot.margin = margin(0, 0, 0, 0, "cm")) +
    geom_rect(aes(xmin = 0, xmax = 0.495, ymin = 0.939, ymax = 1), color = "white", size = 0.5, fill = "grey70")+
    geom_rect(aes(xmin = 0, xmax = 0.04, ymin = 0, ymax = 1), color = "grey70", size = 0.5, fill = "grey70")+
    geom_rect(aes(xmin = 0.495, xmax = 1, ymin = 0.939, ymax = 1), color = "white", size = 0.5, fill = "grey40")+
    geom_rect(aes(xmin = 0.96, xmax = 1, ymin = 0, ymax = 1), color = "grey40", size = 0.5, fill = "grey40") +
    annotate(geom="text", x=0.29, y=0.97, label="(A) NDVI",color="black",size=6)+
    annotate(geom="text", x=0.75, y=0.97, label="(B) LST",color="white",size=6)+
    annotate(geom="text", x=0.02, y=0.74, label="Rio Salado",color="black",size=6,angle=90) +
    annotate(geom="text", x=0.98, y=0.74, label="Rio Salado",color="white",size=6,angle=90) +
    annotate(geom="text", x=0.02, y=0.3, label="New River",color="black",size=6,angle=90) +
    annotate(geom="text", x=0.98, y=0.3, label="New River",color="white",size=6,angle=90)

#close pdf file to save plot to
#dev.off()
```
...
ord.bu
#crop removed because of lack of data in Rio Salado 2010 site
...
Ord_site$Site=as.factor(Ord_site$Site) #sites to factors

#Open a pdf file for saving
#pdf("FIG9.pdf",width=14.5,height=10.5)

#plotting ordination
par(mar = c(5.1, 5.1, 1, 1)) # Set the margin on all sides to 2
plot(bird_ordination,type = "n",xlim=c(-1,1),ylim=c(-0.5,0.5),xlab=expression(paste(bold("NMDS1:")," Temporal Shifts")),ylab=expression(paste(bold("NMDS2:")," Site Differences")),cex.lab=2,cex.axis=1.7)
ordiellipse(bird_ordination,groups=Ord_site$Landscape,draw="polygon", kind="se", conf=0.95,label=FALSE,col = alpha("gray41",0.5),border=alpha("gray65",0.5))
with(Ord_site, points(bird_ordination,display = "sites",cex=4,pch = c(17,16)[Ord_site$Site],col = alpha(c("violetred4","dodgerblue3")[Ord_site$Site],0.8)))
orditorp(bird_ordination, display="species",scaling=-3,air=1,cex=1.4, font=1, col = "gray31")

#plotting vectors
plot(ord.ds,lwd=2,cex=1.7,font=2,border="transparent",bg=c("transparent","transparent","transparent","transparent"),
     col=c("darkgoldenrod3","transparent","transparent","darkgoldenrod3"),
     labels=c("Desert/Soil Size                         ","","","Desert/Soil Distribution                                 "))

plot(ord.veg,lwd=2,cex=1.7,font=2,border="transparent",bg=c("transparent","transparent","transparent","transparent"),
     col=c("darkgreen","transparent","transparent","darkgreen"),labels=c("Veg Size","","","\n\nVeg Distribution                 "))

#plot(ord.veg,lwd=2,cex=1.7,col=c("darkgreen","transparent","transparent","darkgreen"),labels=c("","","",""))

#plot(ord.ds,lwd=2,cex=1.7,font=2,col=c("darkgoldenrod3","transparent","transparent","darkgoldenrod3"),labels=c("","","",""))

plot(ord.bu,lwd=2,cex=1.7,col=c("grey20"),border="transparent",bg=c("transparent","transparent","transparent","transparent"),
     font=2,labels=c("                   Built-up Size","Built-up         \nConnectivity          ","Built-up Shape","Built-up Distribution"))

plot(ord.water,lwd=2,cex=1.7,col=c("darkblue"),font=2,
     labels=c("                  Water Size","Water Connectivity                      ","                      Water Shape","               Water \n               Distribution"))

#bottom arrow showing time
arrows(-0.81,-0.72, 0.83, -0.72, code = 3, lwd = 2, col = 'gray41',lty=2,length=0.1)
arrows(-0.80,-0.72, -0.81, -0.72, code = 2, lwd = 2, col = 'gray41',lty=1,length=0.1)
arrows(0.82,-0.72, 0.83, -0.72, code = 2, lwd = 2, col = 'gray41',lty=1,length=0.1)
text("Earlier Years",x=-0.95,y=-0.72,font=2,cex=1.5, col = 'gray41')
text("Later Years",x=0.95,y=-0.72,font=2,cex=1.5, col = 'gray41')

#Close pdf file for saving
#dev.off()
```
